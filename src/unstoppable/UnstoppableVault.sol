...
contract UnstoppableVault is IERC3156FlashLender, ReentrancyGuard, Owned, ERC4626, Pausable {
     ...
     * @inheritdoc ERC4626
     */
    // the balance of assets held by the ERC4626
     ...
     * @inheritdoc IERC3156FlashLender
     */
    //! missing when not paused
    //! missing non re-entrant modifier, althogh the ReentrancyGuard is imported
        ...
        returns (bool)
    {
        ...
        //@audit BUG convertToAssets should have been here. - // if (convertToAssets(totalSupply) != balanceBefore) revert InvalidBalance();
        // which takes assets as an argument and returns the amount of shares you will get back under ideal conditions (no slippage or fees).
        // totalSupply here represents the total number of shares issued(iven by vault to users), which gets updated only at the time of deposit and withdraw(mint and burn)
        ...
        //@audit not checking the msg.sender
        if (
